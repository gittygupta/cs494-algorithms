...
void main(){
	...
	for (i = 0; i < n; i++)
		printf("%f ", p[i]);
	*/
	// Sorting prices and arranging prices
	...
	*/
	...
	for(i = 0; i < n; i++){
		...
		else if(amount > 0 && amount < w[i]){
			new_vector = amount / w[object_count];
			printf("%f", new_vector);
			w_list[object_count - 1] = w[object_count];
			p_list[object_count - 1] = p[object_count] * new_vector;
			v_list[object_count - 1] = new_vector;
			object_count += 1;
			break;
		}
